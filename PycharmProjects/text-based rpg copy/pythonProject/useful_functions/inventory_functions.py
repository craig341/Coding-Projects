    ...
    if mode == 'use':
        try:
            while True:
                ...
                if not item_index or item_index.isalpha():
                    print()
                    continue
...
def use_inventory():
    ...
    if item_index == -1:  # exit item choice
        return ['exit']
    if item_index.__class__.__name__ in ['Weapon', 'Armour']:  # item is in player hand
        is_player_obj = True
    else:
    ...
    if is_player_obj:
        ...
        if (player.weapon != weapon_dict['Fists'] and item_class == 'Weapon') or (player.armour != armour_dict['Naked'] and item_class == 'Armour'):
    ...
    if item_class == 'Weapon' or item_class == 'Armour':  # weapon and armour cannot be used like potion
...
def summon_item():
    ...
    while True:
        try:
            ...
            add_inv(summon_item_dict[item_name], amount)
